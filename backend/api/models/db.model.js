class dbModal {
  ...
  constructor(dbPath) {
    try {
      ...
      this.Migrations = this.query.Migrations;
      ...
      let runMigration = this.db.transaction(() => {
        try {
          ...
          if (this.migrationLength > userVersion) {
            ...
            this.db.prepare(`PRAGMA legacy_alter_table=ON`).run();
            ...
            this.db.prepare(`PRAGMA foreign_keys = ON`).run();
            this.db.prepare(`PRAGMA legacy_alter_table=OFF`).run();

            this.db.prepare(`PRAGMA user_version = ${this.migrationLength}`).run();
