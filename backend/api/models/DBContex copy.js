class DBContex {
    ...
    run = async (sql, params = null) => {
        return new Promise((resolve, reject) => {
            try {
                let query = this.localDB.prepare(sql);
                let result = query.run(sql, params);
                if (!result) {
                    return reject(err)
                }
                return resolve(this);
    ...
    getList = async (list_name, conditionString = null) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let stmt = this.localDB.prepare(sql);
                let data = stmt.all();
                resolve(data)
            }
            catch (ex) {
    ...
    getAJtypeForConfig = (dept_id) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let stmt = this.localDB.prepare(`${sql1} UNION ${sql2} order by chk`);
                let data = stmt.all(true, false);
                resolve(data)
            }
            catch (err) {
                reject(err);
    ...
    */
        ...
        try {
            if (dataObj) {
                ...
                if (result.changes > 0) {
                    ...
                    selectSql += ` where ${table_name}._id = ${result.lastInsertRowid}`;
                    let selectStmt = this.localDB.prepare(selectSql);
                    let result = selectStmt.get();
                    if (!result) {
                        console.log({ sql: selectSql, result: result });
                        return callback("error occure line no 603 DBContex")
                    }
            ...
            try {
                if (dataObj) {
                    ...
                    this.run(sql, params).then((res) => {
                        ...
                        let stmt = this.localDB.prepare(selectSql);
                        let row = stmt.get();
                        return resolve(row);
    ...
    update = async (tableName, dataObj, conditionString, callback) => {
        try {
            ...
            await this.run(sql, params).then(resolve => {
                ...
                if (!rows) {
                    console.log(sql, err);
                    return callback('error DBconrtex line no 693');
                }
                return callback(null, rows);
            }, reject => {
                return callback('error DBconrtex line no 697');
            });
        }
        catch (ex) {
            return callback(ex)
    ...
    delete = async (tableName, conditionString, callback) => {
        try {
            let stmt = this.localDB.prepare(`DELETE from ${tableName} where ${conditionString}`);
            let result = stmt.run();
            if (!result) {
                return callback('ERROR DBcontex lino no 749')
            }
    ...
    getListForJawak(list_name, dept_id) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let stmt = this.localDB.prepare(sql);
                let result = stmt.all(dept_id);

                if (!result) {
                    console.log(sql, 'line no 786');
                    reject('line no 786')
                }
