...
router.post('/', async (req, res, next) => {
    if (req.body) {
        await DB.insert('subitem_list', req.body, async (err, data) => {
            if (err) {
                return next(err);
            }
            res.json({
...
router.get('/', async (req, res, next) => {
    await DB.getList('subitem_list').then((resolve) => {
        res.json({
            success: true,
            result: resolve || []
        });
...
router.get('/:dept_id', async (req, res, next) => {
    await DB.getFullListByDept('subitem_list', req.params.dept_id).then((resolve) => {
        res.json({
            success: true,
            result: resolve.data || [],
