...
@Component({
  selector: 'app-report-jwk-type-saar',
  templateUrl: './report-jwk-type-saar.component.html',
  styleUrls: ['./report-jwk-type-saar.component.scss']
})
export class ReportJwkTypeSaarComponent {
  ...
  searchReports() {
    ...
    if (!this.filterBody.months || this.filterBody.months.length === 0) {
      this.toastr.error('Please Select atleast one Month');
      return;
    }
    this.isLoader = true;
    this.http.put(this.api.getUrl('REPORT_JT'), this.filterBody).subscribe((data: any) => {
      if (data.success) {
  ...
  exportToPDF() {
    ...
    for (let i in this.reportData) {
      let row: any = {
        ...
        jawak_type_eng: this.reportData[i].jawak_type_eng,
    ...
    let columns = [
      ...
      { header: 'Jawak Type', dataKey: 'jawak_type_eng' },
    ...
    doc.save(this.auth.webUser.dept_eng + '_Jawak_Type_Saar_' + this.monthsSel[0].name + " to " + this.monthsSel[this.monthsSel.length - 1].name + '-' + this.filterBody.year + '.pdf');
  }
  ...
  exportToExcelHin() {
    ...
    for (let i in this.reportData) {
      let row: any = {
        ...
        "जावक टाइप": this.reportData[i].jawak_type_hin,
    ...
    this.excelExportService.exportAsExcelFile(data, this.auth.webUser.dept_code + '_जावक_टाइप_सार_' + this.monthsSel[0].name_hin + " से " + this.monthsSel[this.monthsSel.length - 1].name_hin + '-' + this.filterBody.year);
  }
  ...
  exportToExcelEng() {
    ...
    for (let i in this.reportData) {
      let row: any = {
        ...
        "Jawak Type": this.reportData[i].jawak_type_eng,
    ...
    this.excelExportService.exportAsExcelFile(data, this.auth.webUser.dept_eng + '_Jawak_Type_Saar_' + this.monthsSel[0].name + " to " + this.monthsSel[this.monthsSel.length - 1].name + '-' + this.filterBody.year);
  }
}
